...
def nlasm::order_t() {
	return ptd::var({
			...
			bin_op => @nlasm::bin_op,
			ov_is => ptd::rec({dest => @nlasm::reg_t, src => @nlasm::reg_t, type => ptd::sim(), label_no => ptd::sim()}),
			ov_as => ptd::rec({dest => @nlasm::reg_t, src => @nlasm::reg_t, type => ptd::sim(), label_no => ptd::sim()}),
			return => @nlasm::return,
			die => @nlasm::reg_t,
