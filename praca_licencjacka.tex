...
\lstdefinelanguage{nl}{
  ...
  classoffset=0,
  morekeywords={
    return, match, case, if, elsif, else, while, continue, try, for, fora, forh, rep
  },
...
\lstdefinelanguage{my_c}{
  ...
  classoffset=0,
  morekeywords={
    return, if, else, while, continue, for, rep
  },
  classoffset=1, keywordstyle=\color{purple},
  morekeywords={
	int, void, struct
...
co pomaga w zarządzaniu abstrakcją w programie. Dostępnych jest sześć wbudowanych typów: 
\begin{itemize}
  ...
  \item \texttt{ptd::im} -- typ ogólny, każda zmienna jednego z powyższych typów jest też typu \texttt{ptd::im}
\end{itemize}
...
\begin{itemize}
  ...
  \item[] \textbf{zgodność typów} -- dwa typy są zgodne, jeśli są (być może różnymi) reprezentacjami tego samego typu
\end{itemize}
...
Repozytorium, na którym toczyły się prace, znajduje się pod adresem \cite{repo_dev}.
...
gramatyki języka. Chociaż standardowa definicja drzewa AST mówi wyłącznie o składni, kompilator NianioLanga już na tym etapie dokonuje pewnej
...
Drugi rodzaj analizy stosowanej przez type checker, jest podobny do poprzedniego, lecz w kontekście wnioskowania typów
na podstawie wykonywanych na nich
...
Samą budowę type checker ma podobną w pewnym sensie do parsera. W sposób rekurencyjny sprawdzamy, jakie typy mają poszczególne elementy drzewa oraz
...
\item \texttt{clear} -- sygnalizuje, że wartość licznika referencji powinna zostać zmniejszona o 1
...
dostępów do elementów tablic/haszy/rekordów kończącym się wartością, obliczany jest ciąg zstępujący odwołań do elementów kolekcji. Translator realizuje to za
...
komendy \texttt{call}, bądź, w przypadku rekordów, za pomocą komendy \texttt{key}. Funkcje te tworzą kopię elementu kolekcji, którą zapisują w 
...
\subsection{Uzasadnienie wprowadzenia typów \texttt{own}}
...
Wszystkie inne instrukcje mają ściśle określone typy argumentów i jeśli generator natrafi na instrukcję z argumentem
...
Przykład modyfikacji zmiennej typu \texttt{own}, przedstawionej na listingu \ref{lst:own_access_nl},
przetłumaczonej na nowe polecenia nlasma znajduje się na listingu 
\ref{lst:own_access_nlasm}.

\noindent\begin{minipage}[t]{.50\textwidth}
\begin{lstlisting}[caption={Modyfikacja zmiennej typu \texttt{own} w NianioLangu},language=nl,label={lst:own_access_nl}]
# a : own::rec({
#    b => own::arr(ptd::int()),
# })
...
\end{lstlisting}
\vspace{1em}
W kodzie nlasma z listingu \ref{lst:own_access_nlasm} występuje założenie, że początkowo wartość zmiennej \texttt{a} znajduje się
w rejestrze \texttt{0} i ten rejestr jest jej właścicielem.
W pierwszej kolejności, poprzez polecenie \texttt{:use\_field}, uzyskiwany jest dostęp do pola \texttt{b} w strukturze \texttt{a},
referencja do tego pola umieszczana jest w rejestrze \texttt{2} i ten rejestr staje się nowym właścicelem zmiennej \texttt{a}.

Następnie w analogiczny sposób, przy użyciu poleceń \texttt{:load\_const} i \texttt{:use\_index},
w rejestrze \texttt{4} umieszczana jest referencja do
zerowego elementu tablicy i znów przenoszona jest własność zmiennej \texttt{a}.

Po powyższych operacjach, właścicielem zmiennej \texttt{a} jest rejestr \texttt{4}, który zawiera referencję do modyfikowanej
wartości. Kolejne polecenie, \texttt{:load\_const} umieszcza w nim stałą \texttt{1}.

Ostatnie dwa polecenia \texttt{:release\_*} powodują przeniesienie własności zmiennej \texttt{a} w odwrotnej kolejności.
W ich wyniku właścicielem zmiennej \texttt{a} ponownie zostaje rejestr \texttt{0}.
\end{minipage}\hfill
...
Decyzja o wprowadzeniu pojęcia referencji i właściciela zmiennej do nlasma wiązała się z ryzykiem generowania
...
są kompilowane do tablic typu \texttt{char[]}, opakowanych w struktury zawierające
...
\begin{tabularx}{\textwidth}{|c|R|R|R|}
 ...
 \hline 
  ...
  \multirow{10}{*}{Bez optymalizacji GCC}
  ...
  \hline
  \multirow{10}{*}{Z optymalizacją GCC }
...
\begin{minipage}{\linewidth}
\begin{lstlisting}[caption={Definicje typów \texttt{ptd} do algorytmu merge sort},label={lst:sort_ptd},language=nl]
def main::arr() {
...
\end{lstlisting}
\begin{lstlisting}[caption={Definicje typów \texttt{own} do algorytmu merge sort},label={lst:sort_own},language=nl]
def main::arr() {
...
\end{lstlisting}
\begin{lstlisting}[caption={Algorytm merge sort w NianioLangu},label={lst:sort_nl},language=nl]
def merge_sort(ref arr : @main::arr) {
	merge_sort_aux(ref arr, 0, own_array::len(ref arr) - 1);
}
def merge_sort_aux(ref arr : @main::arr, l : ptd::int(), r : ptd::int()) {
	return if (l == r);
	var mid = (l + r) / 2;
...
\begin{minipage}{\linewidth}
\begin{lstlisting}[caption={Algorytm merge sort w C},label={lst:sort_c},language=my_c]
struct element {
	int x;
	int y;
};
...
pomiędzy różnymi zmiennnymi -- wtedy lepiej sprawdzą się typy \texttt{ptd} ze względu na prostotę i efektywność leniwego kopiowania.

\section{Podsumowanie pracy}
W wyniku przeprowadzonego projektu, do NianioLanga zostały dodane nowe typy, dzięki którym możliwe stało się pisanie w nim efektywnych
programów. Jednocześnie, dzięki włożonej pracy członkowie zespołu poprawili swoje zrozumienie działania kompilatorów
i umiejętności pracy w grupie, takie jak podział zadań i code review.

W trakcie pracy zostały także popełnione błędy. Jednym z nich było zbyt późne i zbyt niedokładne zaprojektowanie mechanizmu zarządzania
pamięcią dla nowych typów. Spowodowało to opóźnienia na ostatnich etapach projektu i wiele problemów ze zwalnianiem pamięci.
Jednak, dzięki wnioskom wyciągniętym z tego projektu, członkom zespołu łatwiej będzie unikać podobnych błędów w przyszłości.

Nowa wersja kompilatora, która powstała w ramach niniejszej pracy, spotkała się z uznaniem ze strony zamawiającej firmy i
ostała umieszczona w oficjalnym repozytorium NianioLanga \cite{repo_prod} jako nowa, badawcza wersja \texttt{nl2}.
W chwili pisania pracy trwają przygotowania do przeniesienia niektórych elementów produkcyjnego oprogramowania napisanego
w NianioLangu, tak żeby wykorzystywały typy \texttt{own}.

\chapter{Wkład poszczególnych osób}
\begin{itemize}
  ...
  \item[] \textbf{Wszyscy członkowie zespołu}
  \begin{itemize}
   \item Korekta pracy licencjackiej
   \item Inpekcja kodu
  \end{itemize}
\end{itemize}
...
\bibitem{autobus}\href{https://main.edu.pl/pl/archive/oi/12/aut}{https://main.edu.pl/pl/archive/oi/12/aut},
data dostępu 22.05.2018
\bibitem{repo_dev} \href{https://github.com/jbujak/nl-zpp}{https://github.com/jbujak/nl-zpp}, data dostępu 07.06.2018
\bibitem{repo_prod} \href{https://github.com/nianiolang/nl2}{https://github.com/nianiolang/nl2}, data dostępu 07.06.2018
