...
Dzięki temu zmniejszymy czas wykonania aplikacji, pozwalając kompilatorowi GCC na zastosowanie dodatkowych optymalizacji.
W tym celu wprowadzone zostają nowe typy z przestrzeni nazw \texttt{own},
...
Jednocześnie zmienione zostają pewne podstawowe typy, mianowicie \texttt{ptd::sim} zostanie rozbity na
\texttt{ptd::int} oraz \texttt{ptd::string} oraz wprowadzony zostaje typ \texttt{ptd::bool}.
...
Skutkiem zmian jakie muszą zostać wprowadzone by osiągnąć cel projektu, będzie utracenie możliwości kompilacji NianioLanga do języków innych niż C, dopóki nie zostaną wprowadzone dodatkowe poprawki.
...
gdyż narzut wydajnościowy związany z tworzeniem obiektów typu im, a potem konstruowanie z nich obiektów typu own byłby zbyt duży.
...
Istnieje parę sposobów na implementację konwersji. Pierwszy sposób polega na zapisaniu w bibliotece C języka ogólnej funkcji konwertujących obiekty \texttt{own} na obiekty \texttt{ptd::im}.
...
Problematyczna jest jednak implementacja dla typów rekurencyjnych (warianty) oraz puchnięcie kodu wynikowego, zawierającego bardzo podobne, potencjalnie bardzo długie sekcje.

Trzeci sposobów, który został zrealizowany, jest niejako połączeniem dwóch poprzednich -- dla każdego typu
...
Najpierw, po załadowaniu drzewa AST modułu do type checkera, przechodzimy po funkcjach definiujących typy, sprawdzając czy nie definiują one typów \texttt{own}.
Miejsce to zostało wybrane, gdyż jest to pierwsze miejsce, gdzie pojawia się konkretna wiedza o typach zmiennych.
Następnie, podczas sprawdzania typów funkcji, za każdym razem gdy natrafimy na funkcję-wydmuszkę zapamiętywany jest typ argumentu, z jakim została wywołana.
...
Ponieważ typy mogą być zagnieżdżone, to czasem trzeba wołać inną funkcję konwertującą wewnętrzny obiekt \texttt{own} na \texttt{im}.
Dla rekordów wypisywane są przypisania wszystkich wartości, \textit{które w kodzie wynikowym C nie mogą być w postaci iteracji po polach rekordu.}
...
który przechodzi przez type checker w sposób podobny do normalnego kodu -- \textit{jest jedynie parę drobnych różnic technicznych polegających na rozwiązaniu problemu,
...
\textit{Doszliśmy do wniosku, że czas kompilacji ma nieduże znaczenie w trakcie pisania programów.}
...
aby każda zmiana miała także i odzwierciedlenie w module generowania kodu.
Zostało przyjęte, że generowane struktury są poprawne, a tylko program użytkownika może zawierać błędy.
