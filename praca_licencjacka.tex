...
wynikowego. Ponadto zachodzi tutaj analiza opisana w rozdziale \nameref{sec:own_to_im}.
...
Drzewo AST, reprezentujące funkcję z języka nianiolang, zostaje w sposób rekurencyjny tłumaczone na ciąg komend języka nlasm. Sposobem działania 
komendy języka nlasm przypominają rozkazy maszynowe: identyfikator + argumenty. Zgodnie ze specyfikacją języka nlasm, każdemu argumentowi funkcji jak 
i każdej zmiennej lokalnej jest przydzielany rejestr odpowiedniego typu. Z powodu struktury języka nlasm, brak w nim jakichkolwiek rozgałęzień kodu, 
...
Tłumaczenie wartości wyrażeń jest bardziej skomplikowane: dla danego wierzchołka w drzewie AST generujemy, wywołania komend, obliczających wartości w 
danym wierzchołku na podstawie podwyrażeń. Na przykład, dla wyrażeń arytmetycznych zapisujemy wartości lewej i prawej strony w rejestrach, po czym 
...
wartość i zapisuje w odpowiednim rejestrze. Dla operacji, posiadających skutki uboczne (np. zapisanie wartośi do zmiennej) stosujemy t.zw. obliczanie 
...
dostępów do elementów talic/haszy/rekordów, kończący się wartością), oblicza ciąg zstępujący odwołań do elementu kolekcji. To translator realizuje za 
pomocą wywołania określonych funkcji bibliotecznych: \texttt{get\_ref\_arr}, \texttt{get\_ref\_hash}, wywołując je z odpowiednimi parametrami za pomocą 
...
początek obliczamy ciąg dostępów do wartości lewej strony, obliczamy prawą stronę, wynik, po czym generuje ciąg przypisań nowych wartości kolejnych 
kolekcji.
...
Jak zostało to opisane w rozdziale \nameref{sec:c_types_implementation}, dotychczasowa implementacja wartości logicznych opierała się na dwuwartościowych
